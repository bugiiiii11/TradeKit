...
| **`handleManualTrade` full path (Session 15)** | UI confirmed working. Order placement path (dashboard → command bus → Hyperliquid) not yet exercised with a real submit. | Bad SL/TP logic, order rejection, or rounding bug could place an unprotected position. Test with small size ($20–30) from Vercel app before relying on it. |
