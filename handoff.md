...
| 5b | **Fix the command-channel 30s flap** (`src/db/commands.ts`) | med | S51 find, see Watchlist. Kill switch degraded to ~30s latency and `bot_logs` is taking ~11k junk rows/day. Reproduce from any machine: query `bot_logs` `source='commands'`. No ssh needed to diagnose; a fix needs a deploy. |
| 5c | **Get ssh access to the VPS** | med | No VPS key exists on the owner's laptop (checked S51: `config`, both `.bak`s, all key files). `ssh ubuntu@170.9.253.98` → `Permission denied (publickey)`. Every Watchlist command is written as `ssh …` and is therefore unrunnable. Owner is not a dev — this is a task for Matt. Possible the box moved (Contabo?); bot logs still show `/home/ubuntu/trading-bot` but that proves layout, not host. |
